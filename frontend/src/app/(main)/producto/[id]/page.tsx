      ...
      if (id) {
        ...
        try {
          const response = await axios.post(
            `${baseUrl}/admin/puntosProducto/producto`,
            {
              id_producto: id,
            }
          )
          if (
            response.data &&
            response.data.puntosProducto &&
            response.data.puntosProducto.cantidadPuntos
          ) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      if (
        isAuthenticated &&
        product.promocion &&
        product.promocion.esValido &&
        product.promocion.limiteStock &&
        product.promocion.limiteStock > 0
      ) {
        const responseGet = await axios.get(
          `${baseUrl}/admin/promocion/${product.promocion.id}`
        )
      ...
      if (
        isAuthenticated &&
        product.promocion &&
        product.promocion.porcentajeDescuento
      ) {
      ...
      if (detalleAnterior) {
        ...
        if (
          ...
          if (nuevoDetalle) {
            ...
            throw new Error(
              "No se pudo crear o actualizar el detalle del producto."
            )
          }
        } else {
    ...
    <div className="flex justify-center items-center">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-6xl flex flex-col lg:flex-row items-center lg:items-start">
        ...
        <div className="lg:w-1/2 w-full lg:pl-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-800 text-center lg:text-left">
            {product.nombre}
          </h1>
          <div className="space-y-4 text-left">
            ...
            <p className="text-gray-700 text-lg">
              ...
              {product.tipoProducto?.nombre || "Sin categoría"}
            </p>
            <p className="text-gray-700 text-lg">
              <strong>Información Nutricional:</strong>{" "}
              {product.informacionNutricional || "No disponible"}
            </p>
            {/*ACAAA*/}
            <p className="text-gray-700 text-lg">
            ...
            {isAuthenticated && existenPuntos && (
              <p className="text-gray-700 text-lg">
                <strong>Puntos Canjeables por compra de producto:</strong>{" "}
                {puntos}
              </p>
            )}
